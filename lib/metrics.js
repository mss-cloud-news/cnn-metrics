...
Metrics.prototype.init = function (opts) {
    ...
    const platform = (process.env.KUBERNETES_PORT) ? 'kubernetes' : 'localhost',
    ...
    if (platform === 'kubernetes') {
        hostname= process.env.HOSTNAME || 'unknown-kub';
    } else {
        hostname='localhost';
    }
    ...
    prefix = util.format('.%s.%s.%s.%s.', platform,  environment, product, this.opts.app);
